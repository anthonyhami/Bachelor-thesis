...
def return_df_hist( df, REAL_OR_FAKE, class_name, bins=20, density=False) :
    df_hist = pd.DataFrame( columns =  ["sample_ID", "DNA_meth_bin", "num_CpGs", "Real_or_Fake"])
...
for exp in range(36):
    ...
    os.makedirs(output_dir_name + "/" + "plot/lines/", exist_ok=True)

    for index_family in range(39):

        df_fake = pd.read_csv(output_dir_name + "/" + "fake_orginal_" + str(index_family) +".csv", sep=";", header=None)
        ...
        df_hist_real= return_df_hist( df_filtered_class_name, REAL_OR_FAKE = "real", class_name=class_name)
        df_hist_fake= return_df_hist( df_fake, REAL_OR_FAKE = "fake" , class_name=class_name)
        ...
        plt.savefig(output_dir_name + "/" + "plot/lines/" + "lineplot_" + class_name +".png")
        plt.clf()


# bsub -eo error.txt -oo output.txt  python histo.py
