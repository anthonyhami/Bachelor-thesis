...
class myCallback(tf.callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}):
        if(logs.get('acc_f') > ACCURACY_THRESHOLD):
...
from tensorflow.keras.callbacks import EarlyStopping
callback = EarlyStopping(monitor='loss', patience=3, verbose=0, mode='min')


"""
from tensorflow.keras import callbacks as cb
class haltCallback(cb.Callback):
    def on_epoch_end(self, epoch, logs={}):
        print ("logs.get('loss') ==--==--==--==--==-- ", logs.get('loss'))
        if(logs.get('loss') <= 2): # 0.05
...
def define_discriminator(in_shape=(976,1), n_classes=39):
    ...
    #model.fit(load_real_samples,epochs=500,callbacks=[callbacks])
    return model
...
def define_generator(latent_dim, n_classes=39):
    ...
    n_nodes = 244*128  # #244*1
    #print("n_nodes.shape==",n_nodes.shape)
    li = Dense(n_nodes, kernel_initializer=init)(li)
    # reshape to additional channel
    li = Reshape((244, 128))(li) #(244,1) to change
    ...
    gen = Reshape((math.ceil(976/4)* 1, 128))(gen) #change( ....,128->1)
...
def load_real_samples():
    trainX = pd.read_csv("976.csv", sep=";",header=None).to_numpy()
    #trainX=trainX[:,:5000]
    ...
    opt = Adam(lr=0.0002, beta_1=0.5)
    #model.compile(loss=['binary_crossentropy', 'sparse_categorical_crossentropy'], optimizer=opt)
    #model.fit(X,trainy,epochs=500,callbacks=[callbacks])
    return [X, trainy]
...
def summarize_performance(step, g_model, d_model, dataset, latent_dim, n_samples=100):
    ...
    filename2 = 'AC_Model_Tumor_callback/model_%04d.h5' % (step+1)
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=500, n_batch=64, patience=300):
    # help variable for patience
    count_patience = 0
    ...
    for i in range(n_steps): #
        ...
        #history = gan_model.fit([z_input, z_labels], [y_gan, z_labels], epochs=1, batch_size=len(z_input), callbacks=[callback], verbose=0) # verbose=1 shows loss on all direction

        #print ("history.history['loss']")
        #print (history)
        #print ("history.history['loss'] ====================", history.history['loss'])
        #print (len(history.history['loss']))
        #print ("avg(history.history['loss']) == ", np.mean(history.history['loss']))

        #print (trainingStopCallback.model.stop_training)
        #print (callback.model.stop_training)
        #exit()
        ...
        #es = EarlyStopping(monitor='acc_f', mode='max', verbose=1, patience=20,baseline=0.5)
        #mc = ModelCheckpoint('best_model.h5', monitor='acc_f', mode='max', verbose=1, save_best_only=True)
        d1_hist.append(d_r)
        d2_hist.append(d_f)
        d3_hist.append(d_r2)
        d4_hist.append(d_f2)
        ...
        if i>900:
            ...
            if d_r > 0.55 and d_f > 0.55:# or previous_d_f <= d_f + 0.02 and previous_d_r <= d_r+0.02: # no improvement
                #print ("previous_d_f <= d_f")
                count_patience = count_patience + 1
            
            elif previous_d_f <= d_f + 0.02 and previous_d_r <= d_r+0.02:
                count_patience = count_patience + 1
            ...
            if count_patience == patience:
                summarize_performance(i - patience, g_patience, d_patience, dataset, latent_dim)
...
#es = EarlyStopping(monitor='d_r', mode='auto', verbose=1, patience=20,baseline=0.5)
#mc = ModelCheckpoint('best_model.h5', monitor='d_r', mode='auto', verbose=1, save_best_only=True,min_delta=1)
